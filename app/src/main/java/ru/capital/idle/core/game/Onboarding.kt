...
object Onboarding {
    ...
    /**
     * Игрок уже проходил игру и в постепенном раскрытии не нуждается.
     *
     * Признак — само перерождение, а не слитки. Раньше ветераном считался любой, у кого
     * есть слитки или престиж-апгрейды, но первые пять слитков выдаёт веха «Первый миллион»,
     * ещё до какого-либо перерождения: на капитале в миллион разом открывалось всё, включая
     * «Престиж» с порогом в миллиард. Нашла это автоматическая симуляция (PR #29) — день 1376
     * у стратегии «только бизнес», день 927 у «только вклады».
     */
    private fun veteran(s: GameState) = s.statLives > 0
     ...
     *
     * Денежные пороги сравниваются с капиталом (`GameMath.netWorth`), а не с валовым доходом:
     * на главном экране показан именно капитал, и расхождение было видно глазами — при
     * капитале 791 464 игрок уже получал доступ к «Миру».
     *
     * Берётся при этом не текущий капитал, а максимальный достигнутый ([GameState.peakNetWorth]):
     * капитал умеет падать — упали акции, продано имущество, — а раз открытый раздел
     * закрываться не должен.
     *
     * Уже объявленный раздел открыт всегда. Это и храповик на будущее, и мост для старых
     * сохранений: там пороги брались от валового дохода, он выше капитала, и без этой строки
     * игрок потерял бы доступ, который у него уже был.
     */
    fun unlocked(s: GameState, id: String): Boolean = veteran(s) || id in s.announced || when (id) {
        "main" -> true
        "dev" -> s.tutorialStep >= 3
        "inv" -> "acc" in s.eduDone
        "world" -> s.peakNetWorth >= 1_000_000.0 || s.milestonesClaimed > 0
        "prof" -> s.tutorialStep >= 2
        // подразделы
        "net" -> s.peakNetWorth >= 5_000.0
        "pres" -> s.peakNetWorth >= 1_000_000_000.0
        else -> true
    }

    /**
     * Замок раздела: заголовок, текст условия и сама проверка этого условия.
     *
     * Текст и проверка лежат рядом не для красоты. Раньше текст был константой, и «Развитие»
     * всегда требовало накопить $ 50, хотя открывается оно по шагу гида: игрок на шаге
     * «устройтесь на работу» с полусотней в кармане читал уже выполненное условие и не
     * понимал, почему раздел закрыт. Теперь [done] — то самое условие, о котором говорит
     * [text], и тест сверяет их между собой.
     */
    data class Lock(val title: String, val text: String, val done: (GameState) -> Boolean)

    /** Что мешает открыть раздел прямо сейчас. */
    fun lock(id: String, s: GameState): Lock = when (id) {
        // «Развитие» и «Профиль» открываются по шагам гида, а шаги идут по очереди:
        // называем то, что не сделано на текущем
        "dev" -> guideLock("Развитие", "раздел откроется", s)
        "prof" -> guideLock("Профиль", "появится ваш профиль и имущество", s)
        "inv" -> Lock("Инвестиции",
            "Пройдите курс «Бухгалтерия» — свободные деньги начнут работать.") { "acc" in it.eduDone }
        "world" -> Lock("Мир",
            "Наберите капитал в $ 1 000 000 — узнаете своё место среди богатейших.") {
            it.peakNetWorth >= 1_000_000.0 || it.milestonesClaimed > 0
        }
        "net" -> Lock("Окружение",
            "Наберите капитал в $ 5 000 — полезные знакомства найдут вас сами.") {
            it.peakNetWorth >= 5_000.0
        }
        "pres" -> Lock("Престиж",
            "Наберите капитал в $ 1 000 000 000 — откроется перерождение.") {
            it.peakNetWorth >= 1_000_000_000.0
        }
        else -> Lock("", "") { true }
    }

    /** Шаг гида, на котором стоит игрок, словами. Хвост — что случится, когда шаг пройден. */
    private fun guideLock(title: String, tail: String, s: GameState): Lock = when (s.tutorialStep) {
        0 -> Lock(title, "Тапните по карте и заработайте первые $ 10 — $tail.") { it.money >= 10.0 }
        1 -> Lock(title, "Устройтесь на работу — $tail.") { it.jobId.isNotEmpty() }
        else -> Lock(title, "Накопите $ 50 на первое обучение — $tail.") { it.money >= 50.0 }
    }

    /** Текст плашки при тапе по замку: заголовок и условие. */
    fun lockText(id: String, s: GameState): Pair<String, String> =
        lock(id, s).let { it.title to it.text }
