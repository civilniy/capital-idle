...
/**
 * Цвет, который старая тема оставляет прежним, а новая берёт по смыслу.
 *
 * Нужен там, где «Стекло» красит одним цветом разные вещи: три полосы распорядка золотые,
 * плашка рынка серая. Менять это в старой теме нельзя, а в новой у каждого смысла свой цвет.
 * Слоты палитры для таких случаев не годятся: значение зависит не от темы, а от места.
 */
@Composable
@ReadOnlyComposable
fun legacy(previous: Color, semantic: Color): Color =
    if (LocalPalette.current.legacyColors) previous else semantic
