...
class EnterpriseNamesTest {
    ...
    @Test
    fun `ни одно название не длиннее лимита ручного ввода`() {
        // случайное предложение не должно быть длиннее того, что игрок может набрать сам:
        // иначе такое имя сохраняется, а вручную его не повторить и не отредактировать.
        // Лимит берётся из константы — если её поменяют, тест поедет за ней
        industryIds.forEach { id ->
            EnterpriseNames.all(id).forEach { n ->
                assertTrue(
                    "отрасль $id: «$n» — ${n.length} знаков при лимите ${EnterpriseNames.MAX_NAME_LEN}",
                    n.length <= EnterpriseNames.MAX_NAME_LEN
                )
            }
        }
    }
