...
object Invariants {
     ...
     *
     * **Список сейчас пуст.** Единственная запись — «слиток за веху делает игрока ветераном,
     * и на капитале в миллион открывается всё, включая „Престиж“ с порогом в миллиард» —
     * убрана вместе с починкой: ветераном теперь считается только тот, кто перерождался.
     */
    fun isKnownFinding(v: Violation): Boolean = false
